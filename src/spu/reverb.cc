...
int PCSX::SPU::impl::AnotherReverbTest(int ns) {
    ...
    if (iCnt & 1)  // we work on every second left value: downsample to 22 khz
    {
        ...
        if (spuCtrl & ControlFlags::ReverbMasterEnable)  // -> reverb on? oki
        {
            ...
            int32_t gIIR = rvb.IIR_ALPHA / 32768L;
            int32_t gCOMB1 = rvb.ACC_COEF_A / 32768L;
            int32_t gCOMB2 = rvb.ACC_COEF_B / 32768L;
            int32_t gCOMB3 = rvb.ACC_COEF_C / 32768L;
            int32_t gCOMB4 = rvb.ACC_COEF_D / 32768L;
            int32_t gWALL = rvb.IIR_COEF / 32768L;
            int32_t gAPF1 = rvb.FB_ALPHA / 32768L;
            int32_t gAPF2 = rvb.FB_X / 32768L;
            ...
            int32_t gLIN = rvb.IN_COEF_L / 32768L;
            int32_t gRIN = rvb.IN_COEF_R / 32768L;
            ...
            s_bufferdrhell(z0_Lapf1, L_apf1 / 32768L);
            s_bufferdrhell(z0_Rapf1, R_apf1 / 32768L);
            s_bufferdrhell(z0_Lapf2, L_apf2 / 32768L);
            s_bufferdrhell(z0_Rapf2, R_apf2 / 32768L);

            rvb.iLastRVBLeft = rvb.iRVBLeft;
            rvb.iLastRVBRight = rvb.iRVBRight;

            rvb.iRVBLeft = ((L_in * rvb.VolLeft) / 3) / 0x4000;
            rvb.iRVBRight = ((R_in * rvb.VolRight) / 3) / 0x4000;
